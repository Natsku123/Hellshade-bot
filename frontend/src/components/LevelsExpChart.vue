...
export default {
  ...
  mounted () {
    ...
    this.gradient2 = this.$refs.canvas
        .getContext('2d')
        .createLinearGradient(0, 0, 0, 450)
    this.gradient.addColorStop(0, hexToRGB(this.$vuetify.theme.currentTheme.primary, 0.5)); // 0.5
    this.gradient.addColorStop(0.5, hexToRGB(this.$vuetify.theme.currentTheme.primary, 0.25)); // 0.25
    this.gradient.addColorStop(1, hexToRGB(this.$vuetify.theme.currentTheme.primary, 0)); // 0

    this.gradient2.addColorStop(0, hexToRGB(this.$vuetify.theme.currentTheme.secondary, 0.9)); // 0.9
    this.gradient2.addColorStop(0.5, hexToRGB(this.$vuetify.theme.currentTheme.secondary, 0.25)); // 0.25
    this.gradient2.addColorStop(1, hexToRGB(this.$vuetify.theme.currentTheme.secondary, 0)); // 0
    this.chartdata = {
      labels: [],
      datasets: [
        {
    ...
    client.request(query).then((data) => {
      ...
      temp = temp.slice(this.range.min, this.range.max);
      temp.forEach((point) => {
        ...
        .then(() => {
          ...
          client.request(membersQuery).then((data) => {
            ...
            tempArr = tempArr.slice(this.range.min, this.range.max);
