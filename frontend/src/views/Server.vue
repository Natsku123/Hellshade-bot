<template>
  <div class="server">
    <v-row justify="center"><v-col><h1>{{ server.name }}</h1></v-col></v-row>
    <v-row justify="center"><v-col><h2>Top 10</h2></v-col></v-row>
    <v-row>
      <v-col md="3" v-for="member in top10" :key="member.uuid" class="">
        <MemberCard :member="member" />
      </v-col>
    </v-row>
    <v-row justify="center"><v-col><h2>All Members</h2></v-col></v-row>
    <v-row>
  ...
  methods: {
    loadContent() {
      ...
      client.request(serverQuery).then((data) => {
        data['server'][0].uuid = data['server'][0].uuid.substring(1, data['server'][0].uuid.length-1);
      ...
      client.request(membersQuery).then((data) => {
        ...
        this.members = data.member.filter(member => !member.player.hidden && member.player.name !== "UNKNOWN");
        ...
        this.top10 = data.member.sort((a, b) => {
